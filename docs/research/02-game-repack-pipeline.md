...
| **zpaqfranz** | Journaling dedup archiver (zpaq fork) | **Actively maintained** (v64.x, releases through 2026) | Built-in dedup + strong methods + integrity checks; MIT-ish (GPL for some parts); good modern replacement for SREP+archiver stages |
...
- Safe-to-bundle set: 7-Zip (LGPL), Zstandard (BSD), zpaqfranz (GPL), FFmpeg builds (GPL),
  Precomp (Apache 2.0), xtool (GPL on GitHub — but depends on what codecs are invoked).
